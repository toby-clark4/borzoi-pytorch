...
TRACKS_DF = pd.read_table(str(DIR / "precomputed" / "targets.txt")).rename(
...
class AnnotatedBorzoi(Borzoi):
    def __init__(self, config, tracks_df=TRACKS_DF):
        ...
        tracks_df["track_transform"] = tracks_df["sum_stat"].apply(
            lambda x: 3 / 4 if x == "sum_sqrt" else 1.0
        )
    ...
    def _build_annotation_df(self, tracks_df):
        ...
        self.sense_tracks = torch.tensor(
            tracks_df.loc[
                tracks_df.identifier.str.contains(r"\+")
                | (tracks_df.index == tracks_df["strand_pair"])
            ].index
        )
        self.antisense_tracks = torch.tensor(
            tracks_df.loc[
                tracks_df.identifier.str.endswith("-")
                | (tracks_df.index == tracks_df["strand_pair"])
            ].index
        )
        # check that ordering of sense and antisense is meaningful
        assert (
            tracks_df.iloc[self.antisense_tracks].description.array
            == tracks_df.iloc[self.sense_tracks].description.array
        ).sum() == self.sense_tracks.shape[0]
        # remember backing dataframe
        self.tracks_df = tracks_df
        self.output_tracks_df = tracks_df.loc[
            tracks_df.identifier.str.contains(r"\+")
            | (tracks_df.index == tracks_df["strand_pair"])
        ].reset_index(drop=True)
        self.register_buffer(
            "scale_values",
            torch.from_numpy(self.output_tracks_df.scale.values)
            .float()
            .unsqueeze(0)
            .unsqueeze(-1)
            .to(self.conv_dna.conv_layer.weight.device),
            persistent=False,
        )
        self.register_buffer(
            "clip_values",
            torch.from_numpy(self.output_tracks_df.clip_soft.values)
            .float()
            .unsqueeze(0)
            .unsqueeze(-1)
            .to(self.conv_dna.conv_layer.weight.device),
        ...
        self.register_buffer(
            "track_transform",
            torch.from_numpy(self.output_tracks_df.track_transform.values)
            .float()
            .unsqueeze(0)
            .unsqueeze(-1)
            .to(self.conv_dna.conv_layer.weight.device),
    ...
    def set_track_subset(self, track_subset):
        ...
        try:
            tracks_df["strand_pair"] = tracks_df["strand_pair"].apply(
                lambda x: oldidx_to_newidx[x]
            )
    ...
    def _predict_gene_count(self, x, gene_slices, predict_antisense=False):
        """
        ...
        if predict_antisense:
            ...
            # adapt the bins
            gene_slices = [
                (self.crop.target_length - 1 - slices) for slices in gene_slices
            ]
        # run model
    ...
    def _undo_squashed_scale(self, x, old_transform=True, unscale=True):
        ...
        if old_transform:
            ...
            x[unclip_mask] = (x[unclip_mask] - clip_soft[unclip_mask]) ** 2 + clip_soft[
                unclip_mask
            ]
            ...
            unclip_mask = x > clip_soft
            x[unclip_mask] = (
                (x[unclip_mask] - clip_soft[unclip_mask] + 1) ** 2
                + clip_soft[unclip_mask]
                - 1
            )
    ...
    def predict_gene_count(
        ...
        if gene_slices is None:  # predict everything if no slices provided
            gene_slices = [
                torch.tensor([x for x in range(self.crop.target_length)])
                for i in range(x.shape[0])
            ]
        pred_sense, slice_length = self._predict_gene_count(x, gene_slices)
        if average_strands:
            pred_antisense, slice_length = self._predict_gene_count(
                x, gene_slices, predict_antisense=True
            )
        ...
        pred = torch.stack(
            [agg_fn(x[0]) for x in torch.split(pred, slice_length, dim=2)]
        )
        if log1p:
